(function() {
  ...
  Hydra.Views.OperationsModal = Backbone.View.extend({
    ...
    onInvoke: function() {
      ...
      setLogActivity('Invoke an operation');
  ...
  Hydra.Client = {
    ...
    invokeRequest: function(method, url, data, headers) {
      ...
      var jqxhr = $.ajax('hydra_console/proxy.php?debug=true&url=' + encodeURI(url), settings);
    ...
    showDocumentation: function(url) {
      ...
      var jqxhr = $.getJSON('hydra_console/proxy.php', { 'url': vocabUrl, 'vocab': 1 }, function(resource) {
        //self.vent.trigger('response', { resource: resource });
...
function setLogActivity(action) {
