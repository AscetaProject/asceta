...
// Plugin Settings
//========================================
define("WPR_USERS_PLUGIN_DB_TABLE",$wpdb->prefix . "wpr_users");
...
// Load Widget
//========================================
require_once WP_PLUGIN_DIR."/wp-restful-users/wp-restful-users-widget.php";
...
function wpr_users_install() {
	...
	//!!!!!!!!!!!!!! vvv To ensure wp-restful is loaded before this plugin vvv !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	$wp_path_to_this_file = preg_replace('/(.*)plugins\/(.*)$/', WP_PLUGIN_DIR."/$2", __FILE__);
	$this_plugin = plugin_basename(trim($wp_path_to_this_file));
	$active_plugins = get_option('active_plugins');
	$this_plugin_key = array_search($this_plugin, $active_plugins);
	// if this plugin is not in last position
	if ($this_plugin_key < count($active_plugins)-1) {
		array_splice($active_plugins, $this_plugin_key, 1);
		// Moving this plugin to last position
		array_push($active_plugins, $this_plugin);
		update_option('active_plugins', $active_plugins);
	}
	//!!!!!!!!!!!!!! ^^^ To ensure wp-restful is loaded before this plugin ^^^ !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
